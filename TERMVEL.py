...
st.markdown("Simulate vertical free-fall of an object through air, including aerodynamic drag.")

with st.sidebar:
    rho = st.number_input("Air Density (kg/m³)", min_value=0.1, max_value=2.0, value=1.225, step=0.01)
    mass = st.number_input("Mass (kg)", min_value=1.0, value=10000.0, step=100.0)
...
while y > 0:
    ...
    velocities.append(v)
    times.append(t)
    if len(velocities) > int(1/dt):
        dv = abs(velocities[-1] - velocities[-int(1/dt)])
        if dv < 1e-3:
            break
